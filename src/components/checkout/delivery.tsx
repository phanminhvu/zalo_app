...
const DeliveryForm = () => {
	...
	return (shippingMethods && shippingMethods.length) > 0 ? (
		<>
			<p className='mt-8 text-lg font-medium'>Shipping Methods</p>
			<div className='mt-5 grid gap-6'>
				{shippingMethods.map((method, mIndex) => {
					return (
						<div className='relative' key={`mth_${mIndex}`}>
								...
								<div className='ml-5'>
									...
									{method.settings.cost && (
										<p className='text-slate-500 text-sm leading-6'>
											{convertPrice(Number(method.settings.cost.value))}
										</p>
									)}
