                ...
                <div className="flex px-2 py-2">
                   {/*} <span className="text-gray-400 mr-3 uppercase text-xs">Brand</span>*/}
                    <div className={"flex-1"}>
                        <p className="text-sm text-black line-clamp-v2  ">{product.name}</p>
                        {(parseFloat(product.sale_price) > 0 || (parseFloat(product.price) > 0 )) && <div className="flex items-center">
                            {(product.on_sale == 1 && product.sale_price > 0) && <del className=" mr-2">
                                <p className="text-xs text-gray-600 cursor-auto ">{convertPrice(product.price || 0)} đ</p>
                            </del>}
                            <p className="text-xs font-bold text-sky-500 cursor-auto">{convertPrice((product.on_sale == 1 && product.sale_price > 0) ? product.sale_price : product.price)} đ</p>
                        </div>}
                    </div>
