...
const CardProductVertical = ({ product, canAdd, padding = 0, grid = false }: CardProductVerticalProps) => {
	...
	return (
		<div
			...
			<img src={pathImg} alt={product.name} className='w-full aspect-auto rounded-lg ' />
					...
					{(parseFloat(product.sale_price) > 0 || parseFloat(product.price) > 0) && (
						<div className='flex items-center'>
							{product.on_sale == 1 && product.sale_price > 0 && (
								<del className='mr-2'>
									<p className='text-xs text-gray-600 cursor-auto font-lato '>{convertPrice(product.price || 0)}đ</p>
								</del>
							)}
