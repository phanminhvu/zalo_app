...
const ProductDescription = ({ text, height = 220 }) => {
	...
	return (
		<div className={`relative overflow-hidden`} style={{ height: isExpand === false ? `${height}px` : `auto` }}>
			<div ref={ref}>{renderHTML(text)}</div>

			{heightContent > height && (
				<div
					className={`${isExpand === false ? `absolute top-[190px]` : `relative bottom-0 left-0`} text-center  w-full`}>
					<Button
						prefixIcon={<Icon icon={isExpand === false ? `zi-chevron-down` : `zi-chevron-up`}></Icon>}
						onClick={() => {
						...
						className={`relative text-pink-400 width-[120]`}
						variant='tertiary'
						size='large'>
