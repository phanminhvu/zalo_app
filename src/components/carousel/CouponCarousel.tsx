...
const CouponCarousel = ({coupons}) => {
    ...
    return (<Glider
        ...
        exactWidth
    >
        {coupons && coupons.map((coupon,cIndex) => (
            <CardCouponHorizontal key={`cc_${cIndex}`} onClick={()=>{setSelectedCoupon(coupon)}} active={selectedCoupon.id === coupon.id} coupon={coupon}/>
        ))}
    </Glider>)
}
