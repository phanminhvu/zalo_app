					...
					<Button
						...
						onClick={async () => {
							await setCart((oldCart) => {
								let nCart = { ...oldCart }
								nCart.cartItems = nCart.cartItems.filter(
									(item) => item.product_id !== product.id && item.parent !== product.id,
								)
								nCart.totalCart = 0
							...
							<Box>
								...
								{products?.filter((rlItem) => product?.related_products?.includes(rlItem.id))
									...
									products
										...
										?.map((rrItem, rindex) => {
											return (
												<div key={`prpick_${rindex}`} className={'flex mb-1'}>
													<Checkbox
														value={rrItem.id}
														onChange={(e) => {
															setRelateItems((old) => {
														...
														checked={relateItems?.some((rlItem) => rlItem.product_id === rrItem.id)}
													/>
													<Text className={'flex-1 pl-2'}>{rrItem.name}</Text>
												</div>
											)
										})}
							</Box>
