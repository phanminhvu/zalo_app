    ...
    return (userAddresses &&
        ...
        >
            ...
            >
                {<List className=" zui-container-background-color py-4">
                    {userAddresses.map((address, index) => {
                        return (<Item key={`address${index}`} className={`mb-4 p-4 bg-white`} title={'Địa chỉ'}
                                      ...
                                      subTitle={address.address} children={<Text
                            className={'zaui-list-item-subtitle'}>{address.name + ' ' + address.phone}</Text>}
                                      onClick={() => {
                                          setShippingAddress(address);
                                          setOpenSheet(false)
                                      }}
                                      suffix={<Button variant={`tertiary`}
                ...
                <Button className={`h-10  px-0 py-2 w-10/12 border-l-0 border-b-0 rounded-full ml-auto mr-auto block`}
                        onClick={async () => {
