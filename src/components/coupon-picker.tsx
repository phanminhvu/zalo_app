...
const CouponsPicker = () => {
	...
	const [openSheet, setOpenSheet] = useRecoilState<boolean>(openCouponPickerState)
				...
				<Sheet
					...
					title={
						<div className={'m-4'}>
							...
							<Input
								suffix={
									<Button style={{ borderRadius: '8px' }} className={'border-none border'}>
										Áp dụng
									</Button>
								}
					...
					<div className='overflow-y-auto max-h-full'>
						{coupons?.filter((c) => c.status === 1)?.length > 0 && (
							<List>
								{/*zi-check-circle*/}
								<Item title={<Text bold size={'xLarge'}>{`Voucher của bạn`}</Text>} />
								{coupons
									.filter((c) => c.status === 1)
									.map((coupon, index) => {
										return (
						...
						{!coupons?.filter((c) => c.status === 1)?.length && (
							<div className={'py-8 px-4'}>
								<Text>{`Hiện tại chưa có mã giảm giá nào`}</Text>
							</div>
						)}
					</div>
				</Sheet>
