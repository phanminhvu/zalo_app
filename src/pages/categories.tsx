...
const CategoriesPage = () => {
    ...
    return (<Container  className={"bg-white px-4 "}>
        <div className="w-full border-b border-gray-200 overflow-auto" >
            {categories && <nav className="flex space-x-2 overscroll-x-auto " aria-label="Tabs" role="tablist">
                {categories.filter(cat=>cat.parent === 0).map((category: Category,cIndex: number) => {
                    return <button key={`cat_${cIndex}`} type="button"
                                   className={`font-semibold ${(activeCategory && activeCategory.id === category.id) ? 'text-primary  border-b border-primary' : ''}  py-4 px-1 inline-flex items-center gap-2 border-b-[3px] border-transparent text-sm whitespace-nowrap text-gray-500 `}
