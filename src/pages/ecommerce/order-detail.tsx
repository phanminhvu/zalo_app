...
const OrderDetail = () => {
    ...
    return <Page className="bg-gray-200 overflow-auto pb-20">
            ...
            <Box className="p-3">
                ...
                */}
                {orderShipType === 0 ? <Box>
                    ...
                    <Text className="font-regular">{`${order.shipping?.name ?? '#'}`}<span className="text-sm text-gray-500 ml-2">{order.shipping?.phone ?? ''}</span>
                    </Text>
        ...
        <Box className="rounded-lg bg-white m-4 overflow-hidden p-3">
            ...
            {order.line_items?.map(item => {
                return (<Box className="flex flex-row mt-3 mb-3">
            ...
            <div className="h-px bg-gray-100 mt-2 mb-2" />
            <Box className="mt-3">
                <Text className="text-end">{`Thành tiền (${order.payment_method}): `}<span className="font-semibold">{convertPrice(order.total)} đ</span></Text>
            </Box>
        </Box>
        <Box className="rounded-lg bg-white m-4 overflow-hidden p-3">
            <Text className="font-medium">{`Bạn cần hỗ trợ?`}</Text>
            <Button className="mt-4" size="medium" fullWidth variant="secondary" onClick={() => {
                 openChat({
                    type: 'oa',
                    id: ZALO_OA_ID,
