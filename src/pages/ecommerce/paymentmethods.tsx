...
const PaymentMethods = () => {
    ...
    return (<Container>
        {loading && (<div className="text-center pt-10"><Spinner
            size="xl"
        /></div>)}
        {(paymentMethods && !loading) && <List className="bg-white">
            {paymentMethods.map((payment,index) => {
                return payment.enabled ? (<Item key={`payment${index}`} className={`mb-0`} title={payment.title} onClick={() => {
                    setSelectedPaymentMethod(payment);
                    navigate('/checkout');
                }}
                suffix={(selectedPaymentMethod && selectedPaymentMethod.id === payment.id) ? <Icon icon="zi-check" /> : <></>}/>) : (<></>)
            })}
        </List>}
    </Container>);
}
