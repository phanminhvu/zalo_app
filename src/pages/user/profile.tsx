...
const UserProfile = () => {
    ...
    return (<Container className={'  zui-container-background-color'}>
        ...
        <Box m={4} p={0} className={"rounded-lg bg-white"}>
            <List>
                ...
                <Item title="Hướng dẫn sử dụng" suffix={<Icon icon="zi-chevron-right" />} className={"text-sm m-0"}  onClick={()=> {
                 ...
                 <Item title="Thêm Quế Quân vào màn hình chính" suffix={<Icon icon="zi-share-external-1" />} className={"text-sm m-0"}  onClick={()=> {
        ...
        >
            <List>
            <Item title="Liên hệ với nhà phát triển ứng dụng" suffix={<Icon icon="zi-chat" />} className={"text-sm m-0"}  onClick={()=> {
                   openUrlInWebview()
                }} />
            </List>
        </Box>
