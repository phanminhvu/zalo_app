	...
	useEffect(() => {
		setHeader({
			customTitle:
				addressAuto == true
					? 'Tìm địa chỉ'
					: userEditingAddress && userEditingAddress?.id > 0
					? 'Sửa địa chỉ'
					: 'Thêm địa chỉ mới',
			hasLeftIcon: true,
			type: 'secondary',
		...
		<Container className={'zui-container-background-color'}>
			<Box mt={1} className={'zui-container-background-color pt-4 '}>
				...
				{!addressAuto && userEditingAddress?.address && (
					<div
						className=' w-11/12 bg-white rounded-lg ml-auto mr-auto p-4 flex'
						onClick={() => {
							setAddressAuto(true)
						}}>
						<HiLocationMarker className='mr-2 h-5 w-5 inline-block' />
						<Text>{userEditingAddress?.address}</Text>
						<Icon icon='zi-chevron-right' size={24} />
					</div>
				)}

				{!addressAuto && userEditingAddress?.address && (
					<Box className={'bg-white p-4 mt-4'}>
						<div className={' mt-4'}>
				...
				{!error && !addressAuto && userEditingAddress?.id && userEditingAddress?.address ? (
					<Button
						variant={`tertiary`}
				...
				{!error && !addressAuto && userEditingAddress?.address && (
					<Button
						...
						<Icon icon='zi-plus-circle' size={24} /> {`Lưu thông tin`}
					</Button>
				)}
			</Box>
		</Container>
