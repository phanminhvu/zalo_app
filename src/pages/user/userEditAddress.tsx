...
const UserEditAddress = () => {
    ...
    return (<Container className={"zui-container-background-color"}>
        <Box mt={1} className={"zui-container-background-color pt-4 "}>
            ...
            {(!addressAuto && userEditingAddress?.address) && <div className=" w-11/12 bg-white rounded-lg ml-auto mr-auto p-4 flex" onClick={()=>{
                setAddressAuto(true);
            }}><HiLocationMarker className="mr-2 h-5 w-5 inline-block" /> <Text>{userEditingAddress?.address}</Text><Icon icon="zi-chevron-right" size={24} /></div>}
            ...
            {(!addressAuto && userEditingAddress?.id  && userEditingAddress?.address ) &&<Button variant={`tertiary`} className={'border-l-0 border-b-0 ml-auto mr-auto block mt-6 px-0 py-2 text-sm  text-rose-600'} onClick={()=>{
            ...
            }}><Icon icon="zi-delete"  /> Xoá địa chỉ</Button>}
            {(!addressAuto && userEditingAddress?.address) && <Button className={`h-10 w-10/12 border-l-0 border-b-0 rounded-full ml-auto mr-auto block mt-6 px-0 py-2`} onClick={async ()=>{
